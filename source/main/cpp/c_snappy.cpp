...
#define kmax32 5
        ...
        static inline void append(sink_t *s, const char *data, size_t n)
        {
            ...
            while ((n -= nlen) > 0)
            {
                ...
                iov++;
                nlen = math::g_min(iov->iov_len, n);
                nmem::memcpy(iov->iov_base, data, nlen);
                s->curoff = nlen;
            }
        }
...
#define kmax_increment_copy_overflow 10
         ...
         */
        size_t max_compressed_length(size_t source_len) { return 32 + source_len + source_len / 6; }
        ...
        static inline char *emit_literal(char *op, const char *literal, int len, bool allow_fast_path)
        {
            ...
            if (n < 60)
            {
                ...
                *op++ = LITERAL | (n << 2);
                 ...
                 * call to nmem::memcpy is overkill. This fast path can sometimes
        ...
        static u16 *get_hash_table(env_t *env, size_t input_size, int *table_size)
        {
            ...
            *table_size = htsize;
            nmem::memset(table, 0, htsize * sizeof(*table));
            return table;
        }
 ...
 */
        ...
        static inline int find_match_length(const char *s1, const char *s2, const char *s2_limit)
        {
            ...
            while (likely(s2 <= s2_limit - 8))
            {
                ...
                else
                {
                     ...
                     */
                    u64 x             = unaligned_load64(s2) ^ unaligned_load64(s1 + matched);
        ...
        static inline int find_match_length(const char *s1, const char *s2, const char *s2_limit)
        {
            ...
            {
                u32 x             = unaligned_load32(s2) ^ unaligned_load32(s1 + matched);
            ...
            if (likely(input_size >= kinput_margin_bytes))
            {
                ...
                for (next_hash = hash(++ip, shift);;)
                {
                    ...
                    do
                    {
                        ...
                        u32 prev_hash    = hash_bytes(get_u32_at_offset(input_bytes, 0), shift);
                        table[prev_hash] = ip - baseip - 1;
                        u32 cur_hash     = hash_bytes(get_u32_at_offset(input_bytes, 1), shift);
                        candidate        = baseip + table[cur_hash];
                        candidate_bytes  = unaligned_load32(candidate);
                        table[cur_hash]  = ip - baseip;
                    } while (get_u32_at_offset(input_bytes, 1) == candidate_bytes);

                    next_hash = hash_bytes(get_u32_at_offset(input_bytes, 2), shift);
                    ++ip;
                }
            }
        ...
        static const u32 wordmask[] = {0u, 0xffu, 0xffffu, 0xffffffu, 0xffffffffu};
        /*
         ...
         */
        static const u16 char_table[256] = {0x0001, 0x0804, 0x1001, 0x2001, 0x0002, 0x0805, 0x1002, 0x2002, 0x0003, 0x0806, 0x1003, 0x2003, 0x0004, 0x0807, 0x1004, 0x2004, 0x0005, 0x0808, 0x1005, 0x2005, 0x0006, 0x0809, 0x1006, 0x2006, 0x0007, 0x080a,
                                            0x1007, 0x2007, 0x0008, 0x080b, 0x1008, 0x2008, 0x0009, 0x0904, 0x1009, 0x2009, 0x000a, 0x0905, 0x100a, 0x200a, 0x000b, 0x0906, 0x100b, 0x200b, 0x000c, 0x0907, 0x100c, 0x200c, 0x000d, 0x0908, 0x100d, 0x200d,
        ...
        ip = d->ip;           \
            ...
            for (;;)
            {
                ...
                if ((c & 0x3) == LITERAL)
                {
                    ...
                    if (writer_try_fast_append(writer_t, ip, d->ip_limit - ip, literal_length))
                    {
                    ...
                    {
                        if (!writer_append(writer_t, ip, avail))
                            return;
                    ...
                    if (!writer_append(writer_t, ip, literal_length))
                        return;
                    ip += literal_length;
                    MAYBE_REFILL();
                }
                else
                {
                    ...
                    const u32 trailer = get_unaligned_le32(ip) & wordmask[entry >> 11];
                    ...
                    if (!writer_append_from_self(writer_t, copy_offset + trailer, length))
                        return;
                    MAYBE_REFILL();
                }
            }
...
#undef MAYBE_REFILL
        ...
        static bool refill_tag(decompressor_t *d)
        {
            ...
            if (nbuf < needed)
            {
                 ...
                 */
                nmem::memmove(d->scratch, ip, nbuf);
                skip(d->reader, d->peeked); /* All peeked bytes are used up */
                d->peeked = 0;
                while (nbuf < needed)
                {
                    ...
                    nmem::memcpy(d->scratch + nbuf, src, to_add);
                    nbuf += to_add;
        ...
        static int internal_uncompress(source_t *r, writer_t *writer_t, u32 max_len)
        {
            ...
            init_decompressor(&decompressor, r);
            ...
            writer_set_expected_length(writer_t, uncompressed_len);

            /* Process the entire input */
            decompress_all_tags(&decompressor, writer_t);

            exit_decompressor(&decompressor);
            if (decompressor.eof && writer_check_length(writer_t))
                return 0;
            return -1;
        }
        ...
        static inline int compress(env_t *env, source_t *reader, sink_t *writer_t)
        {
            ...
            while (N > 0)
            {
                ...
                const unsigned num_to_read = math::g_min(N, kblock_size);
                ...
                {
                    nmem::memcpy(env->scratch, fragment, bytes_read);
        ...
        int compress_iov(env_t *env, iovec *iov_in, int iov_in_len, size_t input_length, iovec *iov_out, int *iov_out_len, size_t *compressed_length)
        {
            ...
            int      err      = compress(env, &reader, &writer_t);

            *iov_out_len = writer_t.curvec + 1;

            /* Compute how many bytes were added */
            *compressed_length = writer_t.written;
            return err;
        }

        /**
         * compress - Compress a buffer using the snappy compressor.
         ...
         *
         * Requires a preallocated environment from init_env.
        ...
        /**
         * uncompress - Uncompress a snappy compressed buffer
        ...
        int compress(env_t *env, const char *input, size_t input_length, char *compressed, size_t *compressed_length)
        {
            ...
            // Compute how many bytes were added
            *compressed_length = (writer_t.dest - compressed);
            return err;
        }
...
#ifdef SG
        /**
         * init_env_sg - Allocate snappy compression environment
        ...
        int init_env_sg(env_t *env, bool sg)
        {
            ...
            if (sg)
            {
                ...
                if (!env->scratch)
                    goto error;
                env->scratch_output = vmalloc(snappy_max_compressed_length(kblock_size));
        ...
        // free_env - Free an snappy compression environment
